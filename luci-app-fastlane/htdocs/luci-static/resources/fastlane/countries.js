...
// ISO 3166-1 alpha-2 codes supported by Xray GeoIP. Country names are
// localized by the browser; the stable code remains visible as a fallback.
var codes = [
...
return baseclass.extend({
	...
	options: function(selected) {
		return codes.map(function(code) {
			return E('option', { value: code, selected: code === selected ? 'selected' : null }, [ displayName(code) + ' (' + code + ')' ]);
		}).sort(function(a, b) {
			return String(a.children[0]).localeCompare(String(b.children[0]), locale());
		});
	}
});
