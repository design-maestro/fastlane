...
@container(max-width:850px){.fl-toolbar-actions{grid-template-columns:1fr 1fr}.fl-search-wrap{grid-column:1/-1}.fl-toolbar-actions .fl-select:last-child{grid-column:1/-1}}@container(max-width:520px){.fl-toolbar-actions,.fl-toolbar-buttons{grid-template-columns:1fr}.fl-search-wrap,.fl-toolbar-actions .fl-select:last-child{grid-column:auto}}
...
// The desktop status grid is deliberately wide. Reassert the mobile layout after
// desktop overrides so a long server name cannot create a horizontal page scroll.
css += '@media(max-width:760px){.fl-status{grid-template-columns:minmax(0,1fr) minmax(0,1fr);min-height:0}.fl-status-cell{min-width:0;padding:10px 12px}.fl-status-cell:first-child{grid-column:1/-1}.fl-status-cell:nth-child(4){grid-column:1/-1;border-right:0}.fl-status-cell-label{flex:0 0 auto;font-size:13px}.fl-status-cell-value{min-width:0;font-size:14px}.fl-mode-switch{grid-column:1/-1;width:auto;margin:12px}.fl-status-disconnect{grid-column:1/-1;width:auto;margin:0 12px 12px}.fl-sourcebar{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));min-height:0;overflow:hidden}.fl-tabs,.fl-source-actions{display:contents}.fl-tab,.fl-source-actions .fl-button{min-width:0!important;min-height:84px;padding:13px 12px;border:0;border-right:1px solid var(--fl-line);border-bottom:1px solid var(--fl-line);border-radius:0;box-shadow:none;white-space:normal}.fl-tab{display:block}.fl-tab-top{gap:7px;font-size:15px;min-width:0}.fl-tab-meta{font-size:12px;min-width:0}.fl-source-actions .fl-button{justify-content:center;gap:5px;font-size:14px;line-height:1.15;text-align:center}.fl-source-actions .fl-button .fl-icon{width:18px;height:18px}.fl-source-add{font-weight:700}}';
...
return view.extend({
	...
	handleRemoveServer: function(subID, nodeID, ev) {
		if (ev) { ev.preventDefault(); ev.stopPropagation(); }
		this.activeMenuKey = '';
		if (!window.confirm(_('Remove this server? If it is active, the VPN will disconnect.'))) {
			this.update();
			return Promise.resolve();
		}
		return this.runAction(
			_('Removing server…'),
			this.exec([ 'remove', subID, '--node', nodeID ]),
			_('Server removed.')
		);
	},
