...
- Даёт опциональный HTTP control plane с авторизацией для будущего отдельного клиента Fast Lane без запуска второго VPN-движка.
...
```mermaid
flowchart LR
    UI[LuCI / CLI / TUI / опциональный HTTP API] --> APP[Сервис Fast Lane]
...
Оригинальные части Fast Lane доступны с исходным кодом по лицензии
[PolyForm Noncommercial 1.0.0](LICENSE). Личное и другое некоммерческое
использование, изменение и распространение разрешены. Для коммерческого
использования нужно отдельное письменное разрешение владельца Fast Lane —
Никиты Горячева (`design-maestro`).

Часть кода создана на основе более раннего проекта под MIT. Обязательное
уведомление сохранено отдельно в
[LICENSES/UPSTREAM-MIT.txt](LICENSES/UPSTREAM-MIT.txt). Точная граница лицензий
описана в [NOTICE](NOTICE). Из-за ограничения коммерческого использования Fast
Lane не является open-source-проектом по определению OSI.
