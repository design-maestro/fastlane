...
func TestFastLaneCurrentViewsKeepMobileInteractionContracts(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "VPN",
			path: filepath.Join("view", "fastlane", "vpn-20260918-country-v31.js"),
			required: []string{
				...
				".fl-status{grid-template-columns:minmax(0,1fr) minmax(0,1fr);min-height:0}",
				".fl-status-cell:first-child{grid-column:1/-1}",
				".fl-sourcebar{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));min-height:0;overflow:hidden}",
				".fl-tabs,.fl-source-actions{display:contents}",
				".fl-tab,.fl-source-actions .fl-button{min-width:0!important;min-height:84px",
		...
		{
			name: "Settings",
			path: filepath.Join("view", "fastlane", "settings-20260918-v10.js"),
			required: []string{
